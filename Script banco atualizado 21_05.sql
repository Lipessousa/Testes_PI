...
for each row 
begin
	insert into HistoricoAgendamento(IdAgendamentoOrigem, IdCliente, NomeCliente, IdServico, NomeServico, TipoAtendimento, DataAtendimento, PrecoAtendimento)
    select IdAgendamento, old.IdCliente, NomeCliente, IdServico, NomeServico, TipoAtendimento, DataAtendimento, PrecoAtendimento
    from Agendamento
...
select a.IdAgendamento, a.IdCliente, a.NomeCliente, a.IdServico, a.NomeServico,  a.TipoAtendimento, a.DataAtendimento, a.PrecoAtendimento
from agendamento a
where a.DataAtendimento < concat(substr(now(),1,4),substr(now(),5,4),substr(now(),9,2)-1, ' ', substr(now(),11,9))
union all
select h.IdAgendamentoOrigem, h.IdCliente, h.NomeCliente, h.IdServico, h.NomeServico,  h.TipoAtendimento, h.DataAtendimento, h.PrecoAtendimento
